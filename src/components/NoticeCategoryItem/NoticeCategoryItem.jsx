...
import { DateItem, Female, FilterStatus, Heart, HeartIcon, ImageContainer, Location, LocationItem, Male,  NoticeList, PetImage, SexItem, LoadMoreBtn, NoticeText, NoticeItem, PetInfo,  } from './NoticeCategoryItem.styled';
    ...
    <Grid container spacing={{ mobile: 1, tablet: 2, desktop: 2 }}>
      {noticesListInfo !== undefined && (
        ...
        noticesListInfo.map(item => (
          <NoticeItem item key={item._id} mobile={12} tablet={6} desktop={3} >
          ...
          {/* <div><HiHeart /></div> */}
            ...
            {/* <PetInfo> */}
          <LocationItem> <Location />{item.location}</LocationItem>
            <DateItem><HiOutlineClock /> {
              

              item.date
            
            }</DateItem>
          <SexItem>{item.sex === "female" ? <Female /> : <Male />} {item.sex}</SexItem>
          {/* </PetInfo> */}
